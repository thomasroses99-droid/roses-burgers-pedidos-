...
import {
  ...
  saveOrder, fmt, MEDALLONES_DEFAULT,
} from "./datos.js";
...
function esDiaPromo() {
  const d = new Date().getDay(); // 0=Dom, 1=Lun, …, 4=Jue
...
function ModalBurger({ burger, fotoUrl, extras, acompList, fotoExtras, medallones, onAgregar, onCerrar }) {
  const disponiblesMedallon = medallones.filter(m => m.disponible);
  const [tamano, setTamano]     = useState("simple");
  const [medallon, setMedallon] = useState(() => disponiblesMedallon[0]?.id || "carne");
    ...
    <div style={{ position: "fixed", inset: 0, background: "#0007", zIndex: 500, display: "flex", alignItems: "flex-end" }} onClick={onCerrar}>
      <div style={{ background: "#FDFAF5", borderRadius: "22px 22px 0 0", width: "100%", maxWidth: 480, margin: "0 auto", maxHeight: "92vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: "18px 20px 36px" }}>
          ...
          {/* Medallón */}
          {disponiblesMedallon.length > 0 && (
            <Bloque titulo="MEDALLÓN">
              <div style={{ display: "flex", gap: 8 }}>
                {disponiblesMedallon.map(op => (
                  <button key={op.id} onClick={() => setMedallon(op.id)}
                    style={{ flex: 1, padding: "12px 4px", borderRadius: 12, border: `2px solid ${medallon === op.id ? G : "#D4CEC6"}`, background: medallon === op.id ? GL : "#fff", cursor: "pointer", textAlign: "center" }}>
                    <div style={{ fontWeight: 800, fontSize: 14, color: medallon === op.id ? GD : "#555" }}>{op.emoji} {op.nombre}</div>
                  </button>
                ))}
              </div>
            </Bloque>
...
function PantallaCheckout({ carrito, onQuitar, tipo, setTipo, zona, envios, onConfirmar, onExito }) {
  ...
  const inp = { width: "100%", border: "1.5px solid #D4CEC6", borderRadius: 12, padding: "14px 16px", fontSize: 15, outline: "none", background: "#FDFAF5", boxSizing: "border-box", color: GD };
          ...
          return (
            <div key={item.cartId} style={{ background: "#FDFAF5", borderRadius: 12, padding: "12px 14px", marginBottom: 8, display: "flex", alignItems: "flex-start", gap: 10, boxShadow: "0 1px 4px #0001", border: tieneDesc ? `1.5px solid ${G}` : "none" }}>
              ...
              <div style={{ textAlign: "right" }}>
                {tieneDesc && <div style={{ fontSize: 11, color: "#bbb", textDecoration: "line-through" }}>{fmt(item.precio)}</div>}
                <div style={{ fontWeight: 700, color: tieneDesc ? G : G, fontSize: 14 }}>{fmt(pConPromo)}</div>
              </div>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              {(envios || []).map(env => (
                <button key={env.id} onClick={() => setLocalidad(env.id)}
                  style={{ flex: 1, padding: "13px 8px", borderRadius: 12, border: `2px solid ${localidad === env.id ? G : "#D4CEC6"}`, background: localidad === env.id ? GL : "#fff", cursor: "pointer", textAlign: "center" }}>
        ...
        <button
          onClick={async () => {
            if (valido && cooldown === 0 && !procesando.current && !guardando) {
              ...
              try {
                setErrorWa(null);
                const result = await onConfirmar({ nombre, telefono, dir, localidad, costoEnvio, localidadNombre: envioObj?.nombre || "", tipo, pago, notas, coords, promoActiva });
                onExito(result);
              } catch (e) {
...
export default function PaginaCliente() {
  ...
  if (!enHorario) return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#1a3a25", padding: "32px 24px", textAlign: "center" }}>
      ...
      <div style={{ fontSize: 26, fontWeight: 900, color: "#a8e6bc", marginBottom: 10 }}>Roses Burgers</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: GL, fontFamily: BODY, maxWidth: 480, margin: "0 auto", position: "relative" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "#000b", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px 20px" }}
          ...
          <div style={{ background: "#FDFAF5", borderRadius: 24, padding: "32px 28px", width: "100%", maxWidth: 380, textAlign: "center", boxShadow: "0 20px 60px #0005" }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
              <div style={{ background: GL, border: `2px solid ${G}`, borderRadius: 16, padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontWeight: 900, fontSize: 16, color: GD }}>Combo Simple</div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>Burger simple + papas fritas</div>
                </div>
                <div style={{ fontWeight: 900, fontSize: 22, color: G }}>$12.000</div>
              </div>
              <div style={{ background: GL, border: `2px solid ${G}`, borderRadius: 16, padding: "14px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontWeight: 900, fontSize: 16, color: GD }}>Combo Doble</div>
                  <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>Burger doble + papas fritas</div>
                </div>
                <div style={{ fontWeight: 900, fontSize: 22, color: G }}>$14.000</div>
              </div>
            </div>
            ...
            <button onClick={() => setShowBanner(false)}
              style={{ width: "100%", background: GD, border: "none", borderRadius: 14, padding: "16px", fontWeight: 900, fontSize: 16, color: "#fff", cursor: "pointer" }}>
              ¡Ver menú!
            </button>
          </div>
        </div>
      ...
      <div style={{ background: GL, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 8px #0001" }}>
        ...
        {pantalla === "menu" && (
          <div style={{ display: "flex", gap: 10, padding: "12px 14px 0" }}>
            {[["delivery", "🛵 Delivery"], ["retiro", "🏠 Retiro"]].map(([v, l]) => (
              <button key={v} onClick={() => setTipo(v)} style={{ flex: 1, padding: "11px", borderRadius: 10, border: `2px solid ${tipo === v ? GD : "#c8c2b8"}`, background: tipo === v ? GD : "transparent", color: tipo === v ? GL : "#888", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                {l}
              </button>
            ))}
          </div>
        ...
        {pantalla === "menu" && (
          <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "10px 14px 12px", scrollbarWidth: "none" }}>
            {CATS.map(c => (
              <button key={c.key} onClick={() => setCat(c.key)} style={{ padding: "7px 16px", borderRadius: 20, border: `1.5px solid ${cat === c.key ? GD : "#c8c2b8"}`, background: cat === c.key ? GD : "transparent", color: cat === c.key ? GL : "#888", fontWeight: 700, fontSize: 13, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                {c.label}
              </button>
            ))}
          </div>
                ...
                return (
                  <div key={g.id}>
                    <div style={{ display: "flex", background: "#FDFAF5", padding: "14px 16px", gap: 14, alignItems: "center" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 800, fontSize: 15, color: GD, marginBottom: 3 }}>{g.nombre}</div>
                ...
                return (
                  <div key={b.id}>
                    <div style={{ display: "flex", background: "#FDFAF5", padding: "14px 16px", gap: 14, alignItems: "center" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 800, fontSize: 15, color: GD, marginBottom: 3 }}>{b.nombre}</div>
      ...
      {/* Modal */}
      {modal && (
        <ModalBurger burger={modal} fotoUrl={getBurgerImg(modal.nombre)} extras={menu.extras} acompList={menu.acomp || []} fotoExtras={fE} medallones={menu.medallones || MEDALLONES_DEFAULT} onAgregar={agregar} onCerrar={() => setModal(null)} />
      )}
    </div>
  );
}
